...
**Overrides land before transform programs.** Program stops resolve against the *fully built* token table: an explicit stop override ([8.5](#85-stop-overrides), e.g. `primary.300: #FF0000`) is visible to every transform program regardless of where in the file (or in which cascade layer) the override appears. A dark-mode mirror therefore lands on the user's overridden stop, never on a frozen copy of the generated ramp — a role's transformed value and a direct read of the stop it lands on can never disagree. The same law covers the untransformed read: where a role's base value is its own base stop, an override of that stop is reflected in the base read too.
